...
| WP-C | Excel ingestion and sheet/row grounded search | done | 2026-06-11 | `python eval/next_stage_wp_c_eval.py` passed: Excel rows converted to records, Search schema uses `zh-Hans.microsoft`, cloud search returned concrete `sheet`/`row` hits for demo and excel corpora, and excel-corpus produced a distinct real model feasibility report. |
