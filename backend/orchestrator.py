...
def _routing_decision_from_llm(req: ChatRequest, raw: dict[str, Any]) -> RoutingDecision:
    ...
    if intent in {"followup_edit", "smalltalk_or_meta", "clarify_needed"}:
        ...
        # Auto-analysis also pulls in market research (Foundry web search) so the
        # capability is exercised on the default demo path, not only when the user
        # explicitly asks for competitors/market.
        if (market_context or auto_analyze) and allow_market_tools and "df-market-researcher" not in experts:
...
def _is_auto_analyze_request(req: ChatRequest) -> bool:
    ...
    if not isinstance(ui_context, dict):
        return False
    return (
        bool(ui_context.get("auto_analyze"))
        or str(ui_context.get("entrypoint") or "") == "workspace_dashboard"
        or str(ui_context.get("mode") or "") == "auto_analysis"
    )


def _iteration_inputs(req: ChatRequest) -> list[dict[str, Any]]:
    """回填指标：从 ui_context.iteration_inputs 读取上一版方案/客户回填的指标。"""
    ui_context = getattr(req, "ui_context", None)
    if not isinstance(ui_context, dict):
        return []
    raw = ui_context.get("iteration_inputs")
...
def extract_plan_metrics(run_id: str) -> dict[str, Any]:
    """从某次分析 run 的方案里抽取【可回填迭代的关键指标】（带缓存）。"""
    try:
        run = get_run(run_id)
    except Exception:
        return {"metrics": [], "error": "run_not_found"}
    artifact = run.get("artifact") or (run.get("final") or {}).get("artifact") or {}
    feasibility = {k: v for k, v in (artifact.get("feasibility") or {}).items() if k != "_llm"}
    final_text = str((run.get("final") or {}).get("text") or "")
    if not feasibility and not final_text:
        return {"metrics": []}
    cache_key = f"plan_metrics:{run_id}"
    ...
    payload = {
        ...
        "dimensions": feasibility.get("dimensions"),
        "gap_list": feasibility.get("gap_list"),
        "plan_text": final_text[:4000],
        "market_note": (artifact.get("market") or {}).get("positioning_note"),
    }
    result = run_plan_metrics_extract(payload)
    try:
...
def _run_feasibility_analyst(
    ...
    if iteration_inputs:
        # 上一版方案回填的指标：明确标注为假设/实测回填，绝不当工作区已证实数据。
        payload["iteration_assumptions"] = {
            "policy": (
                "这些是上一版方案的指标或客户回填的实测/目标值，属【假设或客户回填】，"
                "不是工作区已证实数据。请据此细化方案与敏感性，但任何依赖这些值的结论必须显式标注其为假设/回填，"
                "不得当成 data_confirmed 证据，也不得据此把可行性判得过强。"
            ),
...
def _audit_artifact(req: ChatRequest, artifact: dict[str, Any]) -> tuple[AuditVerdict, dict[str, Any]]:
    ...
    payload = {
        "workspace_id": req.workspace_id,
        "user_request": req.message,
        "revision_round": int(artifact.get("_audit_round") or 0),
...
def _normalize_audit_revision_gate(audit: AuditVerdict) -> tuple[AuditVerdict, str | None]:
    ...
    if any(term in joined for term in blocking_terms):
        return audit, None
    # Strict legacy gate (opt-in): only the blocking keywords above trigger a
    # revision; everything else is downgraded to pass.
    if os.environ.get("DF_AUDIT_STRICT_GATE") == "1":
    ...
    # Trust mode (default): let the auditor's own judgment drive the revision so
    # MAF's conditional loop genuinely participates. A revise verdict is honored
    # whenever it names actionable issues and a target expert; only a verdict
    # with nothing to act on is downgraded. The MAF revision cap keeps it bounded.
    actionable = [issue for issue in audit.issues if str(issue).strip()]
    if actionable and audit.target_expert:
        return audit, None
    adjusted = AuditVerdict(verdict="pass", issues=audit.issues, target_expert=None)
    return adjusted, "empty_audit_no_actionable_issue"
...
async def _orchestrate_chat_impl(req: ChatRequest) -> AsyncIterator[str]:
    ...
    used_maf_audit = False
    if maf_enabled() and "df-auditor" in decision.experts:
        # Microsoft Agent Framework drives the audit ⇄ revise loop as a real
        # workflow graph: the auditor's verdict routes a conditional edge back
        # to the analyst (multi-round) or to finalize. Replaces the legacy
        # single-shot revision below.
        used_maf_audit = True
        max_rev = default_max_revisions()
        yield _frame("maf_workflow", graph_description(max_rev), conv_id)
        yield _frame("role_change", {"agent": "df-auditor", "orchestrator": "maf"}, conv_id)
        try:
            maf_task = asyncio.create_task(
                run_feasibility_audit_loop(working_req, artifact, _run_feasibility_analyst, _audit_artifact)
            )
            async for frame in _progress_frames(maf_task, conv_id, "df-auditor", "maf_audit_loop"):
                yield frame
            maf_state = await maf_task
        except Exception as exc:
            ...
            yield frame
            return
        for step in maf_state.steps:
            node = step.get("node")
            rev = step.get("revision") or 0
            ...
            if node == "reviser":
                ...
                if rev:
                    yield _frame("role_change", {"agent": "df-auditor", "revision": rev, "orchestrator": "maf"}, conv_id)
                rev_extra = {"revision": rev} if rev else {}
                for event, data in _agent_tool_events("df-auditor", meta):
                    yield _frame(event, {**rev_extra, **data}, conv_id)
                if meta.get("response_id"):
                    yield _frame("model_response", {"agent": "df-auditor", **rev_extra, **meta}, conv_id)
                yield _frame("audit", step.get("audit") or {}, conv_id)
        audit = maf_state.audit or AuditVerdict(verdict="pass", issues=[], target_expert=None)
    elif "df-auditor" in decision.experts:
    ...
    if not used_maf_audit and audit.verdict == "revise" and audit.target_expert == "df-feasibility-analyst":
