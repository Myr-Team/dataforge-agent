...
def test_workspace_artifacts_returns_503_for_strict_task_list_failure(tmp_path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    install_workspace_memberships(
        monkeypatch,
        {"ws-artifacts": [active_member("task-owner-oid", "task-tenant", "owner")]},
    )
    ...
    response = TestClient(app, raise_server_exceptions=False).get(
        "/api/workspaces/ws-artifacts/artifacts",
        headers=trusted_headers(actor_id="task-owner-oid", tenant_id="task-tenant"),
    )
