...
def test_run_store_sanitizes_nested_artifact_context_pack_metadata(tmp_path, monkeypatch) -> None:
    monkeypatch.setattr(run_store, "RUN_DIR", tmp_path / "runs")
    monkeypatch.setattr(run_store, "upload_blob_json", lambda *args, **kwargs: None)
    monkeypatch.setattr(run_store, "download_blob_json", lambda *args, **kwargs: {})
    run_store._ACTIVE.clear()

    run_store.start_run("run-artifact-context-pack", "ws-a", "Refine the pilot")
    run_store.complete_run(
        "run-artifact-context-pack",
        final={"text": "done"},
        artifact={
            "context_pack": {
                "status": "ready",
                ...
                "workspace_fact_count": 2,
                "audit_constraint_count": 1,
                "profile_revision": "profile-r9",
                "analysis_revision": "analysis-r7",
                "evidence_refs": ["doc-1"],
                "debug_text": "should never persist",
            }
        },
    )

    persisted = run_store.get_run("run-artifact-context-pack")
    assert persisted["artifact"]["context_pack"] == {
        "status": "ready",
        "version": "context-pack-v1",
        "scope": {"workspace_id": "ws-a", "conversation_id": "conv-a"},
        "fingerprint": "fp-456",
    ...
    assert "profile-r9" not in persisted_json
    assert "analysis-r7" not in persisted_json
    assert "doc-1" not in persisted_json
    assert "should never persist" not in persisted_json


def test_followup_uses_legacy_history_when_durable_fact_lookup_fails(monkeypatch) -> None:
    captured_payloads: list[dict[str, object]] = []
    ...
    def _raise_fact_lookup(_conversation_id, workspace_id=None):
        raise RuntimeError(f"secret durable fact failure for {workspace_id}")

    monkeypatch.setattr(orchestrator, "conversation_durable_facts", _raise_fact_lookup)
    ...
    def _fake_followup(payload):
        ...
        return {
            "text": "## 综合判断\n可以继续，但先收窄试点。",
            "mode": "followup_assessment",
            "response_id": "resp-fact-fallback",
            "usage": {"total_tokens": 12},
            "assessment": "needs_more_evidence",
            "gaps": ["Need real conversion"],
            "clarify": "",
    ...
    assert result["context_pack"]["status"] == "fallback"
    assert result["context_pack"]["fallback_reason"] == "conversation_fact_lookup_failed"
    assert result["text"]
    assert captured_payloads[0]["conversation_history"] == [
        {"role": "user", "text": "Original request", "time": "2026-07-22T10:00:00Z"},
        {"role": "assistant", "text": "Previous answer", "time": "2026-07-22T10:01:00Z"},
    ...
    assert "context_pack" not in captured_payloads[0]
    assert "secret durable fact failure" not in json.dumps(result, ensure_ascii=False)
