...
def fetch_runs(api_url: str, bearer_token: str) -> list[dict[str, Any]]:
    ...
    with urlopen(request, timeout=30) as response:  # nosec B310 - caller supplies URL
        return _extract_runs(json.loads(response.read().decode("utf-8")))
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--bearer-token", help="Bearer token for --api-url; never persisted")
    args = parser.parse_args(argv)
    if args.api_url and not args.bearer_token:
        parser.error("--api-url requires --bearer-token")
    if args.bearer_token and not args.api_url:
        parser.error("--bearer-token requires --api-url")
    return args
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.runs:
        ...
        observed_runs = fetch_runs(args.api_url, args.bearer_token)
    else:
        observed_runs = []
